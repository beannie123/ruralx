...
function requireVerified(req, res, next) {
  if (!req.session.userId || !req.session.verified) {
    return res.status(401).json({ error: "Verified login required" });
  }
  next();
}
...
function requireAdmin(req, res, next) {
  ...
  if (!u || u.role !== "admin") {
    return res.status(403).json({
      error: "Department console access required"
    });
  }
...
app.get("/api/csrf", (req, res, next) => {
  ...
  // Do not regenerate it on every GET.
  if (!req.session.csrf) {
...
app.post("/api/auth/signup", authLimiter, csrf, async (req, res) => {
  ...
  const info = db.prepare(
    "INSERT INTO users(name,mobile,email,password_hash,security_pin_hash,role) VALUES(?,?,?,?,?,?)"
  ).run(
    name,
    mobile,
    email || null,
  ...
  req.session.save(err => {
    if (err) {
      ...
      return res.status(500).json({
        error: "Could not save signup session"
      });
    }
...
app.post("/api/auth/verify-otp", authLimiter, csrf, async (req, res) => {
  ...
  const user = db.prepare(
    "SELECT id,name,mobile,email,verified,role FROM users WHERE id=?"
  ).get(userId);
...
req.session.save(err => {
  if (err) {
    ...
    return res.status(500).json({
      error: "Could not save verified session"
    });
  }
...
app.post("/api/auth/logout", csrf, (req, res) =>
...
app.get("/api/applications", requireVerified, (req, res) => {
  const rows = db.prepare(
    "SELECT ref,service,department,status,created_at FROM applications WHERE user_id=? ORDER BY id DESC"
...
app.get("/api/issues", requireVerified, (req,res) => {
  res.json(
    db.prepare(
      "SELECT ref,category,description,latitude,longitude,status,created_at FROM issue_reports WHERE user_id=? ORDER BY id DESC"
    ).all(req.session.userId)
  );
});
...
function aiChatResponse(query, language="en") {
  ...
  return {
    answer,
    primary:matched,
    intent:matched?`Likely need: ${matched.name}`:"General service discovery",
    confidence,
    matchedKeywords,
    recommendations,
    actions,
    explanation:matched?`RuralX identified ${matched.name} from the words in your request and mapped it to ${matched.department}.`:"RuralX needs a little more information before selecting a specific service."
  };
}
